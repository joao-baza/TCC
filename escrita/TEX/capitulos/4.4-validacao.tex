...
Tendo que o passo a passo para consumir via API ou \textit{Python} o projeto seja demasiado técnico, será apenas brevemente comentado abaixo o modo de se fazer, depois havendo maior realce apenas aos retornos e comentários correlatos.

\begin{figure}[H]
    \centering
    \caption{Inicialização da biblioteca de componentes via \textit{Python}}
    \includegraphics[width=\linewidth]{media/c5293e56ed3caecfe6b0d92a42ca3c75.png}
    \fonte{Autoria própria.}
\end{figure}

No \textit{frontend,} basta acessar a aba “\textit{Components}”, vide imagem abaixo, e \textit{scrollar} para baixo para visualizar as funcionalidades.

\begin{figure}[H]
    \centering
    \caption{Inicialização da biblioteca de componentes via \textit{frontend}}
    \includegraphics[width=\linewidth]{media/e3393d1fba9953384d5c6557aaa6c44f.png}
    \fonte{Autoria própria.}
\end{figure}
...
\begin{figure}[H]
    ...
    \includegraphics[width=\linewidth]{media/f48cac882ed5cb664362e665f4e64bfc.png}
    \fonte{Autoria própria.}
\end{figure}

\subsubsection{Valor de Propriedade}

Ao solicitar as propriedades termofísicas para o componente "Water" (Água) nas condições de $3 0 0 K$ e $1 0 1 3 2 5 P a$ ($1 \  a t m$), o sistema retornou valores consistentes com a literatura. A massa específica calculada foi de $9 9 6 , 5 6 \  K g / m^{3}$ e a viscosidade de $8 , 5 4 \cdot 1 0^{- 4} \  P a \cdot s$. Nota-se que o retorno inclui explicitamente as unidades físicas (utilizando a biblioteca \textit{pint}), garantindo a consistência dimensional dos cálculos subsequentes.

\begin{figure}[H]
    \centering
    \caption{Propriedades termofísicas da água calculadas a 300K e 101,3kPa.}
    \includegraphics[width=\linewidth]{media/4638b6b46783360a5e41e93f89f30e44.png}
    \fonte{Autoria própria.}
\end{figure}

\subsubsection{Nome de Propriedades}

A função \textit{get\_property\_names()} expõe o mapeamento interno entre as chaves de solicitação da API (como 'D', 'H', 'V') e suas respectivas descrições físicas e unidades no SI. Esta estrutura organizacional permite que o usuário identifique rapidamente quais variáveis termodinâmicas podem ser requisitadas ao modelo, facilitando a configuração de simulações de balanço de massa e energia.

\begin{figure}[H]
    \centering
    \caption{Mapeamento de chaves de propriedades e suas unidades padronizadas.}
    \includegraphics[width=\linewidth]{media/7ca015749c830f00ce0f6ee14d38cc7b.png}
    \fonte{Autoria própria.}
\end{figure}

\subsubsection{Propriedades Críticas}

A consulta das propriedades críticas para a água retornou uma temperatura crítica ($T_{c}$) de $6 4 7 , 1 \  K$ e uma pressão crítica $( P_{c} )$ de $2 2 , 0 6 \  M P a$. Estes parâmetros fixos são fundamentais para o cálculo de propriedades reduzidas e para a verificação dos limites de fase da substância nas equações de estado. O sistema também forneceu os dados do ponto triplo, expandindo a utilidade da ferramenta para análises de equilíbrio de fases.

\begin{figure}[H]
    \centering
    \caption{Propriedades críticas e do ponto triplo calculadas para a água.}
    \includegraphics[width=\linewidth]{media/ce947c1c4cd7fe454d92886bf3851072.png}
    \fonte{Autoria própria.}
\end{figure}
...
\end{equation}
    ...
    \item \textbf{Status:} [MATCH] (Desvio: 0,00\%)
\end{itemize}
...
O Número de Reynolds ($Re$) foi validado utilizando as duas formas de entrada suportadas pela API: viscosidade dinâmica ($\mu$) e cinemática ($\nu$).
...
Verificou-se a convergência da solução numérica da API comparando o Lado Esquerdo (LHS) e Direito (RHS) da equação.
\begin{itemize}
    \item \textbf{Dados:} $\varepsilon=0,045mm, D=50mm, Re=100.000$
    \item \textbf{\textit{Software} ($f$):} $0,021614$
    \item \textbf{Check:} LHS = 6,802; RHS = 6,802
...
\begin{itemize}
    \item \textbf{Entrada:} Diâmetro Calculado = 45,0 mm. Schedule = SCH40.
...
\begin{itemize}
    \item \textbf{Dados:} $P_{man}=0,5 kgf/cm^2, P_{atm}=1 atm, P_{vap}=0,02 kgf/cm^2, \gamma=9806 N/m^3, h_f=1m, \Delta Z=2m$
    \item \textbf{Termos de Pressão (m.c.a):} $H_{press} = \frac{(0,5+1,033) \cdot 98.066,5}{9.806,6} = 15,33 m$, $H_{vap} = 0,2 m$
    \item \textbf{Cálculo Manual:} $15,33 + 2,0 + 0,11 - 1,0 - 0,2 = 16,24 \ m$
    \item \textbf{\textit{Software}:} $16,24 \ m$
    \item \textbf{Status:} [MATCH] (Desvio: < 0,01\%)
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{\textit{Software}:} $50mm$ \textbf{[MATCH]}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{\textit{Software}:} $34,29mm$ \textbf{[MATCH]}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{\textit{Software}:} $2,0mm$ \textbf{[MATCH]}
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{\textit{Software}:} $100,0mm$ \textbf{[MATCH]}
\end{itemize}
...
\begin{itemize}
    \item \textbf{Corrente 1 (S1):} $100 \ kg/h$ de Água pura ($z_{água}=1,0$).
    \item \textbf{Corrente 2 (S2):} $100 \ kg/h$ de Etanol puro ($z_{etanol}=1,0$).
...
\textbf{Comparação com o Software:}
...
\begin{figure}[H]
    ...
    \includegraphics[width=\linewidth]{media/code_mass_balance.png}
...
\textbf{Validação:}
O software retornou: $F_{Product} = 100 \ mol/s$, com composição $z_A=0,6$ e $z_B=0,4$, confirmando a conservação de massa e estequiometria.

\subsubsection{Divisor de Corrente (Splitter)}
...
\textbf{Validação:}
Conforme apresentado na saída expandida do software (Figura \ref{fig:code_mass_balance_exp}), os valores de vazão foram exatos ($20$ e $80 \ kg/h$) e a composição manteve-se constante, validando a lógica do divisor.

\begin{figure}[H]
    \centering
    \caption{Resultados expandidos da validação de Balanço de Massa (Misturador, Reator e Splitter)}
    \includegraphics[width=\linewidth,height=0.85\textheight,keepaspectratio]{media/code_mass_balance_expanded.png}
...
O módulo de tubulações (\textit{Piping}) atua como um banco de dados de engenharia, fornecendo especificações padronizadas para acessórios, materiais e dimensões de tubos. A validação deste módulo consiste na verificação da conformidade dos dados retornados com as tabelas normativas padrão.
...
\begin{figure}[H]
    \centering
    \caption{Lista parcial de acessórios disponíveis (JSON)}
    \includegraphics[width=\linewidth]{media/Piping_Fittings_List.png}
    \fonte{Autoria própria.}
\end{figure}
...
Para validação, utilizou-se o acessório "90 degrees Elbow long radius" (Cotovelo 90º Raio Longo).

\begin{figure}[H]
    \centering
    \caption{Especificações do acessório Cotovelo 90º Raio Longo (JSON)}
    \includegraphics[width=\linewidth]{media/Fitting_Specs_90_Degrees_Long_Radius.png}
    \fonte{Autoria própria.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \caption{Lista parcial de materiais de tubulação (JSON)}
    \includegraphics[width=\linewidth]{media/Piping_Materials_List.png}
    \fonte{Autoria própria.}
\end{figure}
...
Para validação, verificou-se o material "Commercial steel" (Aço comercial).

\begin{figure}[H]
    \centering
    \caption{Especificações do material Aço Comercial (JSON)}
    \includegraphics[width=\linewidth]{media/Material_Specs_Commercial_Steel.png}
    \fonte{Autoria própria.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \caption{Lista de classes de pressão (Schedules) disponíveis (JSON)}
    \includegraphics[width=\linewidth]{media/Piping_Schedules_List.png}
    \fonte{Autoria própria.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \caption{Lista parcial de diâmetros para SCH40 (JSON)}
    \includegraphics[width=0.6\linewidth]{media/Diameters_List_SCH40.png}
    \fonte{Autoria própria.}
\end{figure}
...
\begin{figure}[H]
    \centering
    \caption{Especificações do tubo SCH40 DN50 (JSON)}
    \includegraphics[width=\linewidth]{media/Diameter_Specs_SCH40_DN50.png}
    \fonte{Autoria própria.}
\end{figure}
...
\subsubsection{CSTR: Reator Tanque Agitado Continuamente}

A equação de projeto (balanço molar) para um CSTR operando em regime estacionário é a Equação \ref{eq:cstr_design}.
...
\subsubsection{PFR: Reator Tubular}

A equação de projeto para um PFR é obtida através do balanço diferencial de massa (Equação \ref{eq:pfr_design}).
...
\begin{table}[H]
    \centering
    \caption{Validação PFR com Reciclo ($R=2,0; X=0,90$)}
    \begin{tabular}{lrrr}
        \toprule
        \textbf{Parâmetro} & \textbf{Cálculo Manual} & \textbf{\textit{Software}} & \textbf{Erro (\%)} \\
        \midrule
