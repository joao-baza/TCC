...
| E3 | Equation | audio.txt:323-339 | final-paper/TEX/chapters/4.2-engenharia-part1.tex; models/reactor.py; routers/reactor.py | Concentration equation, dilution factor, and reference | Corrected | Task 7 changed `eq:conc_stoich` to divide by `\psi`, use `C_{A0}`, and use `\nu_i`/`\nu_A` at `final-paper/TEX/chapters/4.2-engenharia-part1.tex:101-117`; scan found remaining `n_i` only in the rate-law order term |
| E4 | Equation | audio.txt:665-701 | final-paper/TEX/chapters/4.2-engenharia-part1.tex; models/reactor.py | Volumetric variation and dilution-factor symbol consistency | Corrected | Task 7 normalized `\varepsilon`, changed stoichiometric sums to `\nu_P`/`\nu_R`, and changed the total dilution factor to `\psi = (1+\varepsilon X)(P_0/P)(T/T_0)` at `final-paper/TEX/chapters/4.2-engenharia-part1.tex:126-153`; scans found no `\epsilon` or `\varphi` in the revised reactor section |
...
| E6 | Equation | audio.txt:340-368 | final-paper/TEX/chapters/4.2-engenharia-part2.tex; models/reactor.py | Recycle ratio definition and code basis | Corrected | Task 7 kept the PFR recycle design equation and corrected `R` prose to recycle/recirculation ratio, recycled flow divided by fresh feed flow, at `final-paper/TEX/chapters/4.2-engenharia-part2.tex:5-12` |
...
- Decision: Corrected in Task 7. The denominator now uses `\psi`, product/reactant stoichiometry uses the inlet limiting concentration basis, and the remaining `n_i` scan hit is the rate-law order term rather than a stoichiometric coefficient.

### Equation E4

- LaTeX: `final-paper/TEX/chapters/4.2-engenharia-part1.tex:126-153` defines `\varepsilon = y_{A0}(\sum \nu_P - \sum \nu_R)/|\nu_A|`, then defines `\psi = (1 + \varepsilon X)(P_0/P)(T/T_0)` and explains that `\varepsilon` may represent expansion or contraction.
...
- Decision: Corrected in Task 7. Scans found no `\epsilon` or `\varphi` in the revised reactor section, and `\psi` now matches the nomenclature entry.
...
- LaTeX: `final-paper/TEX/chapters/4.2-engenharia-part2.tex:5-12` gives `V = (R+1)F_{A0}\int_{R/(R+1)X}^{X} dX/(-r_A)` and defines `R` as the recycle/recirculation ratio, using recycled flow divided by fresh feed flow on the same basis as the user-facing parameter.
