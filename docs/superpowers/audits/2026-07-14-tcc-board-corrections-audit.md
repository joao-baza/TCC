...
| W5 | Writing | audio.txt:576-616 | final-paper/TEX/chapters/4.1-desenvolvimento.tex | Figure 1 and pilha/stack wording | Corrected | Task 4 changed visible Figure 1 `Piping` wording to tubulação/tubulações, replaced `pilha principal`, and changed `shell de aplicação` to Portuguese prose; Task 9 removed a remaining visible `Piping` validation prose occurrence, leaving only internal media filenames |
...
| W7 | Writing | audio.txt:765-779 | final-paper/TEX/**/*.tex | Search motor computacional/head and equivalent English terms | Corrected | Task 4 replaced `motor computacional`; Task 9 translated remaining visible `head` headings/captions to altura manométrica. Remaining `Head` text is limited to standard `NPSH` acronym expansion and internal filenames |
...
| F2 | Figure | audio.txt:393-402 | final-paper/TEX/chapters/4.5-recursos-didaticos.tex; final-paper/TEX/media/ | Missing-image markers and PDF pages from 102 onward | Corrected | Missing references were replaced with verified media; follow-up cropped the Moody figure to the visible panel area so the highlighted operating point is clear in the PDF |
...
Task 4 current state: Figure 1 now uses source label `fig:tubulacao_interface` and visible caption `Interface gráfica do módulo de cálculos de tubulação`. The current PDF text no longer shows visible `Piping` in that Figure 1 caption/list entry; only the unchanged media filename `cap4_piping_interface.png` retains `piping` internally. At that point, `F2` remained inventoried as missing-image placeholders around the board's page 102 concern and later pages in the same didactic-resources chapter. Task 9 also removed a separate visible `Piping` occurrence from validation prose in `final-paper/TEX/chapters/4.4-validacao.tex`.
...
- F2 replacements: `fig:glossario` now uses `media/cap7_28_glossario_termos_reynolds.png`; `fig:explicacao_teorica` uses newly captured `media/explicacao_teorica.png`; `fig:viz_moody` now uses `media/cap4_moody_ponto_operacional_recorte.png`, cropped from `media/cap7_08_escoamento_fator_atrito_exemplo.png` to focus on the `Diagrama de Moody` panel and the highlighted operating point; `fig:viz_levenspiel` now uses `media/cap7_21_reatores_levenspiel_exemplo.png`; `fig:trilhas` now uses `media/cap7_01_tela_inicial_trilhas.png`.
...
- Final verification: `./final-paper/compile.sh` exited 0 and generated `final-paper/TEX/main.pdf` with 148 pages. The compile log included `media/cap7_28_glossario_termos_reynolds.png`, `media/explicacao_teorica.png`, `media/cap4_moody_ponto_operacional_recorte.png`, `media/cap7_21_reatores_levenspiel_exemplo.png`, and `media/cap7_01_tela_inicial_trilhas.png`.
...
- Moody follow-up render: `pdftoppm -png -f 109 -l 109 final-paper/TEX/main.pdf /tmp/tcc-moody-final-page` showed Figure 60 using the focused crop, with the red operating point and `operação` label clearly visible.
...
- Task 8 figure check: `pdftotext -layout final-paper/TEX/main.pdf /tmp/tcc-final-paper.txt && rg -n "Figura pendente|inserir media|Chemical Engineering calculator" /tmp/tcc-final-paper.txt` returned no matches after replacing the didactic-resource placeholders. Rendered PDF page checks confirmed the theory screenshot, Levenspiel/trilhas/glossary reused screenshots, and the focused Moody crop render without placeholder boxes.
- Task 8 visual review fix: `final-paper/TEX/media/cap4_moody_ponto_operacional_recorte.png` was cropped from the existing Moody example screenshot and used for Figure 60 because the full screenshot placed the operational point too low in the rendered PDF. Rendered page 109 shows the red operation point and `operação` label clearly enough for the caption.
- Task 9 visible term fix: remaining visible `head` captions/subsection were translated to `altura manométrica` in `final-paper/TEX/chapters/4.2-engenharia-part2.tex` and `final-paper/TEX/chapters/4.4-validacao.tex`; a remaining visible `Piping` validation prose occurrence was replaced with `módulo de tubulações`.
- Task 9 final compile: `./slides/compile.sh` exited 0 with `slides/main.pdf` up to date; `./final-paper/compile.sh` exited 0 with `final-paper/TEX/main.pdf` rebuilt and up to date after final visible-term fixes.
- Task 9 PDF metadata: `pdfinfo slides/main.pdf` reports 37 pages; `pdfinfo final-paper/TEX/main.pdf` reports 148 pages.
- Task 9 text extraction: `pdftotext slides/main.pdf /tmp/tcc-slides-final.txt` generated 960 lines; `pdftotext -layout final-paper/TEX/main.pdf /tmp/tcc-final-paper-final.txt` generated 3444 lines.
- Task 9 final PDF term scan: `rg -n -i "monografia|densidade|head|stack|pilha|motor computacional|curva de reação|Chemical Engineering calculator|Piping" /tmp/tcc-slides-final.txt /tmp/tcc-final-paper-final.txt` found no `monografia`, `stack`, `pilha`, `motor computacional`, `curva de reação`, `Chemical Engineering calculator`, or visible `Piping`. Remaining `head` hits are only standard `Net Positive Suction Head`/`NPSH` acronym expansions. Remaining `densidade` hits are property-list/property-name contexts, including `densidade (\rho, massa específica)`.
- Task 9 final source scan: visible `head`/`Piping` prose was removed from final-paper headings, captions, and validation text. Remaining source hits are internal filenames (`cap4_head_*`, `cap4_piping_interface.png`, `validated-pump-headloss.png`), LaTeX commands (`\endhead`), and standard `NPSH` acronym expansions.
- Task 9 McCabe-Thiele scan: final slide text places McCabe-Thiele under `Métodos e visualizações` and in diagram/visualization contexts; no material-property grouping remains.
- Task 9 equation audit scan: `rg -n "\\| E[0-9] \\|.*\\| Open \\||Reference: *$|Decision: *$" docs/superpowers/audits/2026-07-14-tcc-board-corrections-audit.md` returned no matches.
- Remaining justified terms: Task 5 leaves `densidade` only in property-list/property-name contexts, including the qualified CoolProp property-name context; keeps standard `linha de sucção` and McCabe-Thiele `linhas operacionais`. Task 7 resolved the reactor-section stoichiometric-symbol and `\varphi`/`\psi` equation-symbol decisions. Bibliographic reference confirmation for E1-E7 remains limited to cited keys because no local reference book/PDF files were available.
