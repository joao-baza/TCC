...
| W4 | Writing | audio.txt:552-559 | final-paper/TEX/**/*.tex | Search density/massa especifica context by occurrence | Corrected | Task 5 standardized technical `\rho`/kg m^-3 contexts to `massa específica`; kept `densidade` only where naming the CoolProp/database property |
...
| N1 | Nomenclature | audio.txt:644-663 | final-paper/TEX/main.tex; final-paper/TEX/chapters/4.2-engenharia-part1.tex | Symbols used in equations and list of siglas | Corrected with concern | Task 5 added `\nu_i`, renamed `\varepsilon` as coefficient of volumetric variation, and kept `\psi`; Task 6/7 must resolve current `\varphi`/`\psi` formula-symbol conflict |
...
- Density kept: `final-paper/TEX/chapters/4.2-engenharia-part2.tex:366` intentionally keeps `densidade` when naming the property returned by CoolProp, qualified as `densidade (\rho, massa específica)`.
...
- Nomenclature update: `final-paper/TEX/main.tex` now includes `\nu_i`, names `\varepsilon` as `Coeficiente de variação volumétrica da reação`, and keeps `\psi` as `Fator de diluição`.
- Concern deferred to Task 6/7: `final-paper/TEX/chapters/4.2-engenharia-part1.tex` still contains Equation `\ref{eq:dilution_factor}` written with `\varphi`, while surrounding prose and the symbol list use `\psi`. Task 5 did not resolve this formula-symbol conflict by instruction.
...
- Remaining justified terms: Task 5 leaves `densidade` only in the qualified CoolProp property-name context, keeps standard `linha de sucção` and McCabe-Thiele `linhas operacionais`, and defers the `\varphi`/`\psi` equation-symbol conflict to Task 6/7.
