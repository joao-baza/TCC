...
| F2 | Figure | audio.txt:393-402 | final-paper/TEX/chapters/4.5-recursos-didaticos.tex; final-paper/TEX/media/ | Missing-image markers and PDF pages from 102 onward | Corrected | Missing references were replaced with verified media; follow-up changed the Moody figure from the old friction-factor form screenshot to the guide screenshot that includes the Moody panel and highlighted point |
...
- F2 replacements: `fig:glossario` now uses `media/cap7_28_glossario_termos_reynolds.png`; `fig:explicacao_teorica` uses newly captured `media/explicacao_teorica.png`; `fig:viz_moody` now uses `media/cap7_08_escoamento_fator_atrito_exemplo.png` because visual inspection confirmed it contains the `Diagrama de Moody` panel with the operational point highlighted; `fig:viz_levenspiel` now uses `media/cap7_21_reatores_levenspiel_exemplo.png`; `fig:trilhas` now uses `media/cap7_01_tela_inicial_trilhas.png`.
...
- Final verification: `./final-paper/compile.sh` exited 0 and generated `final-paper/TEX/main.pdf` with 148 pages. The compile log included `media/cap7_28_glossario_termos_reynolds.png`, `media/explicacao_teorica.png`, `media/cap7_08_escoamento_fator_atrito_exemplo.png`, `media/cap7_21_reatores_levenspiel_exemplo.png`, and `media/cap7_01_tela_inicial_trilhas.png`.
