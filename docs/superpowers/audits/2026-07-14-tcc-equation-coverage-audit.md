...
| PROP-04 | Properties | `routers/components_router.py:get_props_by_state` | CoolProp generic state call `PropsSI(output, input1, value1, input2, value2, fluid)` | `routers/components_router.py:/components/props-by-state` | Open | CoolProp | Open | Classify as external library documented |
| PROP-05 | Properties | `routers/components_router.py:get_property_surface` | CoolProp property grid over temperature and pressure, with units, min and max | `routers/components_router.py:/components/property-surface` | Open | CoolProp | Open | Classify as external library documented |
| PROP-06 | Properties | `routers/components_router.py:get_saturation_envelope` | CoolProp saturated liquid/vapor pressure, entropy and enthalpy using quality `Q=0/1` | `routers/components_router.py:/components/saturation-envelope` | Open | CoolProp | Open | Classify as external library documented |
| PROP-07 | Properties visualization model | `routers/components_router.py:get_phase_envelope_chart` | phase-envelope chart derived from saturation-envelope domain data | `routers/components_router.py:/components/phase-envelope/chart` | Open | CoolProp | Open | Decide if chart wrapper or domain coverage |
| PROP-08 | Properties visualization model | `routers/components_router.py:get_vapor_pressure_chart` | vapor-pressure curve derived from saturation-envelope domain data | `routers/components_router.py:/components/vapor-pressure/chart` | Open | CoolProp | Open | Decide if chart wrapper or domain coverage |
