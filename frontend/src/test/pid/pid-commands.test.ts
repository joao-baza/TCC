...
describe("comandos canônicos P&ID", () => {
  ...
  it("conecta portas compatíveis e bloqueia direção, capacidade e autorreferência", () => {
    const context = deterministicContext();
    let document = emptyDocument();
    document = applyCommand(document, insertSymbol(symbol, { x: 0, y: 0 }), context);
    document = applyCommand(document, insertSymbol(symbol, { x: 200, y: 0 }), context);
  ...
  it("applyCommand bloqueia utilityCategoryId em aresta process", () => {
    const context = deterministicContext(5300);
    let document = applyCommand(emptyDocument(), addUtilityCategory("X", "red"), context);
    const categoryId = document.metadata.utilityCategories[0].id;

    document = applyCommand(document, insertSymbol(symbol, { x: 0, y: 0 }), context);
    document = applyCommand(document, insertSymbol(symbol, { x: 200, y: 0 }), context);
    const [nodeA, nodeB] = Object.values(document.nodes);
    const out = Object.values(document.ports).find(p => p.nodeId === nodeA.id && p.direction === "output")!;
    const inp = Object.values(document.ports).find(p => p.nodeId === nodeB.id && p.direction === "input")!;
    document = applyCommand(document, connectPorts(out.id, inp.id), context);

    const edgeId = Object.keys(document.edges)[0];
    expect(() => applyCommand(document, patchElement(edgeId, { utilityCategoryId: categoryId }), context))
      .toThrow(/monotônica/);
  });
});
