...
export function CatalogPanel(props: CatalogPanelProps) {
  ...
  const selectedSource = source ?? uncontrolledSource;
  ...
  useLayoutEffect(() => {
    ...
    const button = document.getElementById(pendingFocusId) as HTMLButtonElement | null;
    if (button) {
      button.focus();
  ...
  const changeSource = (next: CatalogSourceKind | undefined) => {
    if (source === undefined) setUncontrolledSource(next);
    onSourceChange?.(next);
  };
          ...
          <div style={{ height: totalSize, position: "relative" }}>
            {visibleItems.map((virtualRow) => {
              ...
              return <div key={row.id} ref={virtualizer.measureElement} data-index={virtualRow.index} style={{ position: "absolute", top: 0, left: 0, width: "100%", transform: `translateY(${virtualRow.start}px)` }}>
                {row.kind === "category" ? <button id={row.id} role="treeitem" aria-level={1} aria-expanded={!collapsed.has(row.category)} tabIndex={activeId === row.id ? 0 : -1} onClick={() => toggleCategory(row.category)} onKeyDown={(event) => onRowKeyDown(event, row, virtualRow.index)} className="min-h-11 w-full px-3 text-left font-medium focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring">{row.category}</button>
                  : <button id={row.id} role="treeitem" aria-level={2} tabIndex={activeId === row.id ? 0 : -1} onClick={() => onInsert(row.symbol)} onKeyDown={(event) => onRowKeyDown(event, row, virtualRow.index)} className="flex min-h-[76px] w-full items-center gap-3 border-t px-3 text-left outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-ring">
                    <img src={row.symbol.assetUrl} alt="" loading="lazy" decoding="async" width={48} height={40} className="h-10 w-12 rounded bg-muted object-contain" />
...
function toRows(symbols: readonly CatalogSymbol[], collapsed: ReadonlySet<string>): readonly CatalogRow[] {
  ...
  for (const symbol of symbols) { const group = categories.get(symbol.category) ?? []; group.push(symbol); categories.set(symbol.category, group); }
  return Object.freeze([...categories.entries()].flatMap(([category, items]) => {
    const canonical = canonicalCategory(category);
    return [{ kind: "category" as const, id: `category:${canonical}`, category }, ...(collapsed.has(category) ? [] : items.map((symbol) => ({ kind: "symbol" as const, id: `symbol:${symbol.key}`, category, symbol })) )];
  }));
}
