...
\par\medskip
\noindent\textbf{3. Anular (Tubo Concêntrico)}
\begin{itemize}
...
\par\medskip
\noindent\textbf{5. Canal Circular (Meia Seção)}
\begin{itemize}
...
\textbf{Validação:}
Conforme sintetizado na representação didática (Tabela \ref{fig:code_mass_balance_exp}), os valores de vazão foram exatos ($20$ e $80 \ kg/h$) e a composição manteve-se constante, validando a lógica do divisor.

\begin{table}[H]
    \centering
    \caption{Representação didática do balanço de massa validado}
    \label{fig:code_mass_balance_exp}
    \footnotesize
    \begin{tabular}{@{}p{0.15\textwidth}p{0.28\textwidth}p{0.31\textwidth}p{0.17\textwidth}@{}}
        \hline
        \textbf{Etapa} & \textbf{Entradas} & \textbf{Processamento} & \textbf{Saídas verificadas} \\
        \hline
        Misturador & $S1=S2=100\ \mathrm{kg/h}$; água e etanol puros. & Soma da vazão global e balanço por componente, sem reação. & $F_{S3}=200\ \mathrm{kg/h}$; $z_{agua}=z_{etanol}=0,5$. \\
        Reator & $F_{A,in}=100\ \mathrm{mol/s}$; $X_A=0,40$; $A\to B$. & Aplicação da estequiometria 1:1: $F_{A,out}=F_{A,in}(1-X_A)$ e $F_{B,out}=F_{A,in}X_A$. & $F_{A,out}=60\ \mathrm{mol/s}$; $F_{B,out}=40\ \mathrm{mol/s}$. \\
        \textit{Splitter} & $F_{in}=100\ \mathrm{kg/h}$; $z_A=z_B=0,5$; fração de reciclo $0,2$. & $F_{Reciclo}=0,2F_{in}$ e $F_{Produto}=(1-0,2)F_{in}$, preservando a composição. & $F_{Reciclo}=20\ \mathrm{kg/h}$; $F_{Produto}=80\ \mathrm{kg/h}$. \\
        \hline
    \end{tabular}
    \fonte{Elaboração própria (2026).}
\end{table}
...
O módulo de tubulações atua como um banco de dados de engenharia, fornecendo especificações padronizadas para acessórios, materiais e dimensões de tubulações. A validação deste módulo consiste na verificação da conformidade dos dados retornados com as tabelas normativas padrão.
...
A função \textit{diameter\_specifications(schedule, dn)} retorna as dimensões reais da tubulação, incluindo diâmetro externo, espessura de parede e diâmetro interno (calculado).
Para validação, utilizou-se uma tubulação de \textit{Schedule} 40 com Diâmetro Nominal 50 (aprox. 2 polegadas).
...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.25\textwidth}p{0.28\textwidth}p{0.31\textwidth}}
        ...
        Especificação de acessório & \parbox[t]{\linewidth}{\centering\textit{fitting\_specifications(}\newline\textit{nome)}} & \parbox[t]{\linewidth}{\raggedright Comprimento equivalente $L_{eq}/D=16$ do cotovelo de 90 graus de raio longo.} \\
        Composição & \textit{compositions()} & Materiais disponíveis e suas rugosidades absolutas. \\
        Especificação de composição & \parbox[t]{\linewidth}{\centering\textit{composition\_}\newline\textit{specifications(}\newline\textit{nome)}} & Aço comercial: rugosidade absoluta $0,06\ \mathrm{mm}$ e coeficiente de rugosidade $C=135$. \\
        ...
        Especificação de diâmetro & \parbox[t]{\linewidth}{\centering\textit{diameter\_specifications(}\newline\textit{schedule, dn)}} & \parbox[t]{\linewidth}{\raggedright SCH40 DN50: diâmetro externo $60,3\ \mathrm{mm}$, espessura $3,91\ \mathrm{mm}$, massa linear $4,44\ \mathrm{kg/m}$ e pressão máxima $7,93\ \mathrm{MPa}$ (catalogada como $1150\ \mathrm{psi}$).} \\
        \hline
    \end{tabular}
