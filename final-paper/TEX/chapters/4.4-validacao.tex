...
\begin{table}[H]
    ...
    \begin{tabular}{p{0.25\textwidth}p{0.28\textwidth}p{0.31\textwidth}}
        ...
        Especificação de acessório & \textit{fitting\_specifications(}\\\textit{nome)} & Comprimento equivalente $L_{eq}/D=16$ do cotovelo de 90 graus de raio longo. \\
        Composição & \textit{compositions()} & Materiais disponíveis e suas rugosidades absolutas. \\
        Especificação de composição & \textit{composition\_}\\\textit{specifications(}\\\textit{nome)} & Aço comercial: rugosidade absoluta $0,06\ \mathrm{mm}$ e coeficiente de rugosidade $C=135$. \\
        ...
        Especificação de diâmetro & \textit{diameter\_specifications(}\\\textit{schedule, dn)} & SCH40 DN50: diâmetro externo $60,3\ \mathrm{mm}$, espessura $3,91\ \mathrm{mm}$, massa linear $4,44\ \mathrm{kg/m}$ e pressão máxima $7,93\ \mathrm{MPa}$ (catalogada como $1150\ \mathrm{psi}$). \\
        \hline
    \end{tabular}
