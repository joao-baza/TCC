...
Nos exemplos de balanço desta seção, as vazões exibidas em cada soma ou subtração usam a mesma unidade de vazão. No misturador, as vazões mássicas estão em $kg/h$; no reator de conversão, as vazões molares estão em $mol/s$ e a conversão é adimensional; no divisor de corrente, as vazões mássicas estão em $kg/h$.
...
Para esta validação, $C_A$ está em $mol/m^3$, $k$ tem unidade $s^{-1}$ por se tratar de reação de primeira ordem, $-r_A$ está em $mol/(m^3\,s)$, $F_{A0}$ está em $mol/s$, $X$ é adimensional e $V$ é retornado em $m^3$.
...
Na validação do \textit{PFR}, $X$ é adimensional; $F_{A0}/(-r_A)$ tem unidade de volume ao combinar vazão molar com taxa molar de reação por volume de reator; portanto, a integral retorna $V$ em $m^3$.
...
Na validação com reciclo, $R$ e $X_{in}$ são adimensionais; a variável de integração $X$ é adimensional; $F_{A0}/(-r_A)$ carrega unidade de volume; os volumes finais do reator são reportados em $m^3$.
