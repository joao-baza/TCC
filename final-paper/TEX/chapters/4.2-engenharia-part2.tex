...
Tendo que $R$ representa a razão de reciclo ou recirculação, definida na mesma base do parâmetro de entrada do sistema como vazão reciclada dividida pela vazão fresca de alimentação. Assim, o termo $( R + 1 )$ acomoda o aumento da vazão que atravessa o reator na integração. No código, essa integral é avaliada numericamente (por exemplo, via \textit{scipy.integrate.quad}). Para o caso inverso, em que $V$ é conhecido e se busca $X$, utiliza-se novamente um procedimento iterativo (método de Brent) aplicando a equação de \textit{design} diferencial até encontrar a conversão cujo volume calculado corresponda ao especificado \cite{fogler2009}.
...
Em que $H$ é a altura manométrica adicionada (se positiva) ou removida (se negativa) entre os pontos 1 e 2. Nessa equação, $\frac{P_{2} - P_{1}}{\rho g}$ representa a variação de pressão estática convertida em altura de coluna de fluido, $z_{2} - z_{1}$ é a variação de cota geométrica, o termo de velocidades representa a variação de energia cinética e $h_{f}$ engloba todas as perdas de carga (distribuídas e localizadas) entre os pontos. Com essa convenção, perdas positivas aumentam a altura requerida pela bomba \cite{white2018}.
